...
export const onChangeTaskStateAction = async (
    _prevState: FormState,
    data: FormData
): Promise<FormState> => {
    const formData = Object.fromEntries(data);
    const validatedFormData = changeTaskStateFormSchema.safeParse(formData);
    ...
    const res = await apiFetch(API_ROUTES.TASK.UPDATE, {
        ...
        body: JSON.stringify({
            ...
            state: validatedFormData.data.state,
        }),
    });

    if (!res.ok) {
        return {
            errorMessage: 'Įvyko klaida keičiant užduoties statusą',
