...
export const onCreateTaskAction = async (
    _prevState: FormState,
    data: FormData
): Promise<FormState> => {
    const formData = Object.fromEntries(data);
    const validatedFormData = createTaskFormSchema.safeParse(formData);
    ...
    const res = await apiFetch(API_ROUTES.TASK.CREATE, {
        ...
        body: JSON.stringify({
            ...
            taskDescription: validatedFormData.data.taskDescription || null,
        }),
    });
